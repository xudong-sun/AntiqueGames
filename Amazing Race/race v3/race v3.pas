...
var
 ...
 soc:array[1..3000] of integer;
 ss:array[1..3000] of integer;
 bs:array[1..3000] of boolean;
 s,sr:string;
 f:text;
procedure print;
var
...
begin
 ...
 for r:=1 to 13 do
  begin
   ...
   k:=random(20)+1;
   str(k,sr);
   s:='d:\little games\amazing race\map v3\'+sr+'.txt';
   assign(f,s); reset(f);
   ...
   read(f,rr);
   close(f);
   s:='d:\little games\amazing race\record v3\'+sr+'.txt';
   assign(f,s); reset(f);
    ...
    repeat
     ...
     for i:=1 to sn do if bs[i] then
      begin
       ...
       x:=ss[i];
       while (x>0) and (de[soc[i]+1]=0) do begin inc(soc[i]); dec(x); end;
